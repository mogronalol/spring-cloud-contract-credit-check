...
public class CreditScoreConsumer {
    ...
    public void publishScore(CreditCheckRequest creditCheckRequest) {
        final CreditCheckResponse creditCheckResponse = creditCheckService.doCreditCheck(creditCheckRequest.getCitizenNumber());
        creditScoreProducer.publishScore(new CreditCheckResponse(creditCheckRequest.getUuid(), creditCheckResponse.getScore()));
    }
}
